...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	for (UINT i = 0; dxgiFactory->EnumAdapterByGpuPreference(i, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE,
		...
		useAdapter = nullptr;//ソフトウェアアダプタの場合見なかったことにする
	}
	...
	hr = device->CreateDescriptorHeap(&rtvDescriptionHeapDesc, IID_PPV_ARGS(&rtvDescriptorHeap));
	assert(SUCCEEDED(hr));
	...
	//現時点でインクルードしないが、includeに対応するための設定をおこなっておく
	...
	*materialData = Vector4(1.0f, 0.0f, 1.0f, 1.0f);
	...
	while (msg.message != WM_QUIT)
	{
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			//Fenceの値を更新
			fenceValue++;
			//GPUがここまでたどり着いたときに、Fenceの値を指定しあt値に代入するようにSignalを送る
